...
{\flushleft \em Construction of MS-PUF. }
...
The sofware generates a challenge in the form of a time-series pattern
comprising of digital values, which are transferred one at a time
...
The software program accumulates 
all ADC outputs to form the response to the challenge. Algorithm~\ref{algo:puf_sw}
has the details.
...
\begin{scriptsize}
	...
	\DontPrintSemicolon
	\KwIn{$\mathbf C$ : an array representing the challenge}
	\KwOut{$\mathbf R$ : an array representing the response}
	\Begin{
		\For{$i \in \{1, 2, \cdots len(\mathbf C)\}$}{
			OutputToDAC($\mathbf C_i$); \\
			/* Wait for ADC to finish conversion */ \;
			$\mathbf R_i$ = InputfromADC(); \;	
		}
		\Return ${\mathbf R}$
	}
\end{scriptsize}
...
Why would we expect to have variation

A DAC output depends on several nano-scale features such 
as variations in voltage references, resistor tolerances, 
capacitive effects, and amplifier differences. This could result 
in two DACs having different responses to the same input. 
Successive approximation ADCs have a in-built DAC, therefore
would be affected by the same variations as the DAC. Additionally,




Better PUFs by considering multiple devices
